...
c    developed to run a single geneation of one configuration 
c    defined Line 199+
...
c     ------------------------------------------
c       open(10,file=inpath//'otb_config.dat',status='old')
c       read(10,*) sidfile
c       read(10,*) root   ! scenario 
c       close(10)
c 122   format(a10)

c       open(10,file=inpath//sidfile,status='old')
c       read(10,*) sid         ! gridid code, from mkrandgrid.f
c       read(10,55) igen,ttest,tmin  ! generation #, (d) for ftest mkrandgrid.f
c       close(10)

c-----------------------------------------------
c      manually define configuration
c----------------------------------------------

       root = 'pt00'  ! single model run
       ilen = len(root)
...
c       open(11,file=inpath//sidfile(1:6)//'_'//sgen,status='replace')
c       write(11,*) sid
c       write(11,*) igen,ttest,tmin
c       close(11)
...
c      if (mod(int-iccctime(1).eq.0) then 
c         iopen=1
c         iwrite=1
c      endif
...
c         write(6,*) 'AT INT=',int 
      write(6,*) 'opening ',outpath//' file for '//
     .        root(1:ilen)//'_'//sid//'.dat'
c      OPEN (IUSAL1,FILE=outpath//'salinity_'//
c     .     ab(iab)//root(1:ilen)//'_'//sid//'.dat',
c     & FORM='unformatted')
