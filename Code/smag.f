...
C
      ...
      DO 110 J=2,JMM1
      DO 110 I=2,IMM1
      AAM(I,J,K)=HORCON*H1(I,J)*H2(I,J)*FSM(I,J)
...
C
C     AAM(I,J,K)=HORCON*H1(I,J)*H2(I,J)*FSM(I,J)
C    .  *SQRT( ((U(I+1,J,K)-U(I,J,K))/H1(I,J))**2
C    .        +((V(I,J+1,K)-V(I,J,K))/H2(I,J))**2
C    . +.5*(.5*(U(I,J+1,K)+U(I+1,J+1,K)-U(I,J-1,K)-U(I+1,J-1,K))
C    .    /H2(I,J)
C    .    +.5*(V(I+1,J,K)+V(I+1,J+1,K)-V(I-1,J,K)-V(I-1,J+1,K))
C    .    /H1(I,J))**2)
 110  CONTINUE
C
      DO 120 N=1,NUMQBC
...
C
      DO 130 J=2,JMM1
      DO 130 I=2,IMM1
 130  AAM2D(I,J)=HORCON*H1(I,J)*H2(I,J)*FSM(I,J)
...
C
C     AAM2D(I,J)=HORCON*H1(I,J)*H2(I,J)*FSM(I,J)
C    .  *SQRT( ((UA(I+1,J)-UA(I,J))/H1(I,J))**2
C    .        +((VA(I,J+1)-VA(I,J))/H2(I,J))**2
C    . +.5*(.5*(UA(I,J+1)+UA(I+1,J+1)-UA(I,J-1)-UA(I+1,J-1))
C    .    /H2(I,J)
C    .    +.5*(VA(I+1,J)+VA(I+1,J+1)-VA(I-1,J)-VA(I-1,J+1))
C    .    /H1(I,J))**2)
C
      DO 112 N=1,NUMQBC
      IC=IQC(N)
